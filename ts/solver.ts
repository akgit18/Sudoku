...
let permaBoard: number[][];
function solveHelper(prevBoard: number[][], i: number, j: number) {
  let solved = false;
...
function isLegalAll(board: number[][], sqList: null | number[][] = null) {
  if (!sqList) {
...
function isLegalSpace(board: number[][], i: number, j: number) {
  let val = board[i][j];
  if (val > _size**2) {
...
//basic rules
function rowcolumn(board: number[][], i: number, j: number, val: number): boolean {
...
function boxrule(board: number[][], i: number, j: number, val: number): boolean {
  let iFloor = Math.floor(i/_size) * _size;
  let jFloor = Math.floor(j/_size) * _size;
...
//additional rules
function knightsmove(board: number[][], i: number, j: number, val: number): boolean {
...
function kingsmove(board: number[][], i: number, j: number, val: number): boolean {
...
function orthoplus(board: number[][], i: number, j: number, val: number): boolean {
  let dx = [-1, 0, 0, 1];
  let dy = [0, -1, 1, 0];
  for (let k = 0; k < 4; k++) {
    let x = i + dx[k];
...
function toggleRule(label: HTMLLabelElement, checkbox: HTMLInputElement) {
  ...
  if (checkbox.checked) {
    ...
    tog = (r: (board: number[][], i: number, j: number, val: number) => boolean) => {rules.push(r)};
  } else {
    label.className="lb1";
    tog = (r: (board: number[][], i: number, j: number, val: number) => boolean) => {rules.splice(rules.indexOf(r), 1)};
  }
  switch (val) {
    case "1":
      tog(knightsmove)
...
function nextEmptySpace(board: number[][]): number[] | undefined {
  for (let i = 0; i < (_size ** 2); i++) {
    for (let j = 0; j < (_size ** 2); j++) {
      if (board[i][j] < 1) {
        return [i, j];
...
function numViolationsSpace(board: number[][], i: number, j: number, val = board[i][j]) {
  if (val > _size**2) {
    throw new Error(`Value ${val} on row ${i+1}, column ${j+1} too large.`)
  }
  let viols = 0;
...
function minViolatingOption(board: number[][], i: number, j: number) {
