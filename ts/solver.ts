type Rule = (board: Readonly<number[][]>, i: number, j: number, val: number) => boolean

const rules: Rule[] = [rowcolumn, boxrule]; //active rules
//var ruleViols = [rowcolumnboxViols] //violation counters

//recursive helper for backtracking
function backtracking(boardRef: number[][], i: number, j: number): boolean {
...
function getBoard() {
  ...
  for (let i = 0; i < (_size ** 2); i++) {
    ...
    for (let j = 0; j < (_size ** 2); j++) {
      let cell = (tr.cells[j].children[0] as any).value;
      if (cell > 0) {
...
function isLegalAll(board: Readonly<number[][]>, sqList: null | number[][] = null) {
  if (!sqList) {
...
function isLegalSpace(board: Readonly<number[][]>, i: number, j: number) {
  let val = board[i][j];
  if (val > _size**2) {
...
//basic rules
function rowcolumn(board: Readonly<number[][]>, i: number, j: number, val: number): boolean {
...
function boxrule(board: Readonly<number[][]>, i: number, j: number, val: number): boolean {
  let iFloor = Math.floor(i/_size) * _size;
  let jFloor = Math.floor(j/_size) * _size;
...
//additional rules
function knightsmove(board: Readonly<number[][]>, i: number, j: number, val: number): boolean {
...
function kingsmove(board: Readonly<number[][]>, i: number, j: number, val: number): boolean {
...
function orthoplus(board: Readonly<number[][]>, i: number, j: number, val: number): boolean {
  let dx = [-1, 0, 0, 1];
  let dy = [0, -1, 1, 0];
  for (let k = 0; k < 4; k++) {
    let x = i + dx[k];
...
function toggleRule(label: HTMLLabelElement, checkbox: HTMLInputElement) {
  ...
  if (checkbox.checked) {
    ...
    tog = (r: Rule) => {rules.push(r)};
  } else {
    label.className="lb1";
    tog = (r: Rule) => {rules.splice(rules.indexOf(r), 1)};
  }
  switch (val) {
    case "1":
      tog(knightsmove)
...
function nextEmptySpace(board: Readonly<number[][]>): number[] | undefined {
  for (let i = 0; i < (_size ** 2); i++) {
    for (let j = 0; j < (_size ** 2); j++) {
      if (board[i][j] < 1) {
        return [i, j];
...
let solver = (board: number[][], i: number, j: number) => backtracking(board, i, j)
function sudokize() {
  try {
    let board = getBoard();
    if (!isLegalAll(board)) {
...
function putBoard(board: Readonly<number[][]>) {
  for (let i = 0; i < (_size ** 2); i++) {
    let tr = table.rows[i];
    for (let j = 0; j < (_size ** 2); j++) {
      let cell = tr.cells[j].children[0];
      (cell as any).value = board[i][j];
    }
  }
}
...
function localSearch(board: number[][], i: number, j: number) {
  //error checking
  if (!isLegalAll(board)) {
    throw new Error("Inputted board not valid.");
  }
...
function numViolationsSpace(board: Readonly<number[][]>, i: number, j: number, val = board[i][j]) {
  if (val > _size**2) {
    throw new Error(`Value ${val} on row ${i+1}, column ${j+1} too large.`)
  }
  let viols = 0;
...
function minViolatingOption(board: Readonly<number[][]>, i: number, j: number) {
...
function boxSq(i: number, j: number) {
  const sqList = [];
...
function rowcolumnSq(i: number, j: number) {
  const sqList = [];
...
function setify(i: number, j: number) {
  let l1 = boxSq(i, j);
